...
// lintSkills structurally validates the embedded skills/ and rules/ trees.
// It returns human-readable problems; an empty slice means clean. It is
// content-agnostic (no personal denylist) so it is safe to commit.
func lintSkills(fsys fs.FS) []string {
	var problems []string
	...
	rules, err := fs.ReadFile(fsys, "rules/herdle.md")
	switch {
	case err != nil:
		problems = append(problems, "rules/herdle.md: missing")
	case strings.TrimSpace(string(rules)) == "":
		problems = append(problems, "rules/herdle.md: empty")
	default:
		if fm, ok := parseFrontmatter(string(rules)); ok {
			if _, has := fm["paths"]; has {
				problems = append(problems, "rules/herdle.md: has paths: key (must load every session)")
			}
		}
	}
